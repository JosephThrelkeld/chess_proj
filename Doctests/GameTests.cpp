...
void printMoveSet(std::set<ChessPosition> moveSet) {
    for (ChessPosition mvmt: moveSet) {
        std::cout << mvmt.row() << " " << mvmt.col() << std::endl;
    }
    std::cout << std::endl;
}
...
TEST_CASE("Basic Game Tests") {
    ...
    CHECK(testGame->returnPossMoves({1, 1}) == std::set<ChessPosition>{{2, 1},
                                                                       {3, 1}});

    testGame->movePiece({1, 1}, {2, 1});

    CHECK(testGame->returnPossMoves({2, 1}) == std::set<ChessPosition>{{3, 1}});
    ...
    CHECK(testGame->returnPossMoves({0, 4}) == std::set<ChessPosition>{{0, 3}});
...
TEST_CASE("Pawn Tests") {
    ...
    CHECK(testGame->returnPossMoves({6, 4}) == std::set<ChessPosition>{{5, 4},
                                                                       {4, 4}});

    testGame->movePiece({6, 4}, {4, 4});

    CHECK(testGame->returnPossMoves({4, 4}) == std::set<ChessPosition>{{3, 4}});
    ...
    CHECK(testGame->returnPossMoves({4, 4}) == std::set<ChessPosition>{{3, 5}});
}
...
TEST_CASE("Knight Tests") {
    ...
    CHECK(testGame->returnPossMoves({7, 1}) == std::set<ChessPosition>{{5, 0},
                                                                       {5, 2}});
    ...
    CHECK(testGame->returnPossMoves({4, 2}) == std::set<ChessPosition>{{6, 1},
                                                                       {6, 3},
                                                                       {5, 0},
                                                                       {3, 0},
                                                                       {2, 1},
...
TEST_CASE("Bishop Tests") {
    ...
    CHECK(testGame->returnPossMoves({7, 2}) == std::set<ChessPosition>{{6, 3},
                                                                       {5, 4},
                                                                       {4, 5},
                                                                       {3, 6},
                                                                       {2, 7}});
    ...
    CHECK(testGame->returnPossMoves({4, 5}) == std::set<ChessPosition>{{5, 6},
                                                                       {3, 4},
                                                                       {2, 3},
                                                                       {1, 2},
                                                                       {7, 2},
...
TEST_CASE("Rook Tests") {
    ...
    CHECK(testGame->returnPossMoves({3, 3}) == std::set<ChessPosition>{{2, 3},
                                                                       {4, 3},
                                                                       {5, 3},
                                                                       {6, 3},
                                                                       {3, 0},
...
TEST_CASE("Queen Tests") {
    ...
    CHECK(testGame->returnPossMoves({3, 3}) == std::set<ChessPosition>{{2, 2},
                                                                       {2, 3},
                                                                       {2, 4},
                                                                       {3, 0},
                                                                       {3, 1},
...
TEST_CASE("King Tests") {
    ...
    CHECK(testGame->returnPossMoves({4, 4}) == std::set<ChessPosition>{{3, 3},
                                                                       {3, 4},
                                                                       {3, 5},
                                                                       {4, 3},
                                                                       {4, 5}});
